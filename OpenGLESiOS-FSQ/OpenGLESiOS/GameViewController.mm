...
GLuint vbo,gpuProgram,texture,ebo,colorBuffer,fbo,fsqProgram,fsqVBO;
GLint posLocation,texcoordLocation,textureLocation,normalLocation,MLocation,VLocation,PLocation,NMLocation,fsqPosLocation,fsqTextureLocation;
...
-(void)initScene
{
    ...
    vsCode=LoadAssetContent("Data/Shader/fsq.vs");
    fsCode=LoadAssetContent("Data/Shader/texture.fs");
    
    fsqProgram=CreateGPUProgram(vsCode,fsCode);
    fsqPosLocation=glGetAttribLocation(fsqProgram, "pos");
    
    char*bmpFile=LoadAssetContent("Data/niutou.bmp");
    ...
    fsqVBO=CreateBufferObject(GL_ARRAY_BUFFER, sizeof(float)*18, fsqData, GL_STATIC_DRAW);
...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect
{
    ...
    glVertexAttribPointer(texcoordLocation, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)(sizeof(float)*3));
    ...
    glVertexAttribPointer(fsqPosLocation, 3, GL_FLOAT, GL_FALSE, sizeof(float)*3, 0);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
